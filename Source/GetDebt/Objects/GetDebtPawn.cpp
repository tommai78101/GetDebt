...
const FName AGetDebtPawn::LookUpBinding("LookUp");
const FName AGetDebtPawn::LookRightBinding("LookRight");
const FName AGetDebtPawn::EngineAudioRPM("RPM");

#define LOCTEXT_NAMESPACE "VehiclePawn"

AGetDebtPawn::AGetDebtPawn(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	...
	// Wheels/Tyres
	// Setup the wheels
	Vehicle4W->WheelSetups[0].WheelClass = UGetDebtWheelFront::StaticClass();
	Vehicle4W->WheelSetups[0].BoneName = FName("PhysWheel_FL");
	Vehicle4W->WheelSetups[0].AdditionalOffset = FVector(0.f, -8.f, 0.f);

	Vehicle4W->WheelSetups[1].WheelClass = UGetDebtWheelFront::StaticClass();
	Vehicle4W->WheelSetups[1].BoneName = FName("PhysWheel_FR");
	Vehicle4W->WheelSetups[1].AdditionalOffset = FVector(0.f, 8.f, 0.f);

	Vehicle4W->WheelSetups[2].WheelClass = UGetDebtWheelRear::StaticClass();
	Vehicle4W->WheelSetups[2].BoneName = FName("PhysWheel_BL");
	Vehicle4W->WheelSetups[2].AdditionalOffset = FVector(0.f, -8.f, 0.f);

	Vehicle4W->WheelSetups[3].WheelClass = UGetDebtWheelRear::StaticClass();
...
void AGetDebtPawn::SetupPlayerInputComponent(class UInputComponent* InputComponent)
{
	// set up gameplay key bindings
	check(InputComponent);

	InputComponent->BindAxis("MoveForward", this, &AGetDebtPawn::MoveForward);
	InputComponent->BindAxis("MoveRight", this, &AGetDebtPawn::MoveRight);
	InputComponent->BindAxis(LookUpBinding);
	InputComponent->BindAxis(LookRightBinding);

	InputComponent->BindAction("Handbrake", IE_Pressed, this, &AGetDebtPawn::OnHandbrakePressed);
	InputComponent->BindAction("Handbrake", IE_Released, this, &AGetDebtPawn::OnHandbrakeReleased);
	InputComponent->BindAction("SwitchCamera", IE_Pressed, this, &AGetDebtPawn::OnToggleCamera);

	InputComponent->BindAction("ResetVR", IE_Pressed, this, &AGetDebtPawn::OnResetVR); 
}

void AGetDebtPawn::MoveForward(float Val)
{
	GetVehicleMovementComponent()->SetThrottleInput(Val);

}

void AGetDebtPawn::MoveRight(float Val)
{
	GetVehicleMovementComponent()->SetSteeringInput(Val);
}

void AGetDebtPawn::OnHandbrakePressed()
{
	GetVehicleMovementComponent()->SetHandbrakeInput(true);
}

void AGetDebtPawn::OnHandbrakeReleased()
{
	GetVehicleMovementComponent()->SetHandbrakeInput(false);
}

void AGetDebtPawn::OnToggleCamera()
{
	EnableIncarView(!bInCarCameraActive);
}

void AGetDebtPawn::EnableIncarView(const bool bState)
{
...
void AGetDebtPawn::Tick(float Delta)
{
...
void AGetDebtPawn::UpdateHUDStrings()
{
...
void AGetDebtPawn::SetupInCarHUD()
{
...
void AGetDebtPawn::UpdatePhysicsMaterial()
{
